...
WINDOWS_WSL_HELPER="${HYOPS_REPO_ROOT}/tools/install/windows/install-windows-wsl.sh"
...
grep -Fq "Start-Process -FilePath 'wsl.exe' -WindowStyle Hidden" "${WINDOWS_LAUNCHER}"
grep -Fq 'Clear-Host' "${WINDOWS_LAUNCHER}"
grep -Fq -- '--noprofile --rcfile' "${WINDOWS_LAUNCHER}"
grep -Fq '.hybridops/config/windows-shell.rc' "${WINDOWS_LAUNCHER}"
grep -Fq "PS1='\\[\\033[36m\\]hyops\\[\\033[0m\\]:\\[\\033[34m\\]\\w\\[\\033[0m\\]\\\$ '" "${WINDOWS_WSL_HELPER}"
...
grep -Fq 'Run Install HybridOps.cmd again to update an existing installation.' \
